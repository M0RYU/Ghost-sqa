...
function isOldVisibilityFormat(visibility) {
  return !Object.prototype.hasOwnProperty.call(visibility, 'web') || !Object.prototype.hasOwnProperty.call(visibility, 'email') || !Object.prototype.hasOwnProperty.call(visibility.web, 'nonMember') || isNullish(visibility.web.memberSegment) || isNullish(visibility.email.memberSegment);
}
function isVisibilityRestricted(visibility) {
  if (isOldVisibilityFormat(visibility)) {
    return visibility.showOnEmail === false || visibility.showOnWeb === false || visibility.emailOnly === true || visibility.segment !== '';
  } else {
...
var visibilityUtils = /*#__PURE__*/Object.freeze({
    ...
    FREE_MEMBERS_SEGMENT: FREE_MEMBERS_SEGMENT,
    NO_MEMBERS_SEGMENT: NO_MEMBERS_SEGMENT,
    PAID_MEMBERS_SEGMENT: PAID_MEMBERS_SEGMENT,
    buildDefaultVisibility: buildDefaultVisibility,
    isOldVisibilityFormat: isOldVisibilityFormat,
...
const getResizedImageDimensions = function (image, {
  width: desiredWidth,
...
function renderImageNode(node, options = {}) {
  ...
  if (defaultMaxWidth && node.width > defaultMaxWidth && isLocalContentImage(node.src, options.siteUrl) && canTransformImage && canTransformImage(node.src)) {
    ...
    const {
      width,
      height
    } = getResizedImageDimensions(imageDimensions, {
  ...
  if (options.target === 'email' && node.width && node.height) {
    ...
    if (node.width >= 600) {
      imageDimensions = getResizedImageDimensions(imageDimensions, {
        width: 600
      });
    }
...
function ctaCardTemplate(dataset) {
  ...
  const buttonStyle = dataset.buttonColor === 'accent' ? `style="color: ${dataset.buttonTextColor};"` : `style="background-color: ${dataset.buttonColor}; color: ${dataset.buttonTextColor};"`;
  return `
...
function emailCTATemplate(dataset, options = {}) {
  const buttonStyle = dataset.buttonColor === 'accent' ? `color: ${dataset.buttonTextColor};` : `background-color: ${dataset.buttonColor}; color: ${dataset.buttonTextColor};`;
  let imageDimensions;
  ...
  if (dataset.layout === 'minimal' && dataset.imageUrl) {
    if (isLocalContentImage(dataset.imageUrl, options.siteUrl) && options.canTransformImage?.(dataset.imageUrl)) {
      ...
      const iconSize = options?.imageOptimization?.internalImageSizes?.['email-cta-minimal-image'] || {
        width: 256,
        height: 256
      }; // default to 256 since we know the image is a square
...
class CallToActionNode extends generateDecoratorNode({
  ...
  // Where colour is customisable, we should use hex values
  {
    ...
    name: 'sponsorLabel',
    default: '<p><span style="white-space: pre-wrap;">SPONSORED</span></p>'
  }, {
    name: 'backgroundColor',
    default: 'grey'
  },
  // Since this is one of a few fixed options, we stick to colour names.
  {
    name: 'imageUrl',
    default: ''
  }, {
    ...
    row.forEach(image => {
      ...
      if (defaultMaxWidth && image.width > defaultMaxWidth && isLocalContentImage(image.src, options.siteUrl) && canTransformImage && canTransformImage(image.src)) {
        const {
          width,
          height
        } = getResizedImageDimensions(image, {
          width: defaultMaxWidth
        });
        img.setAttribute('width', width);
        img.setAttribute('height', height);
      ...
      if (options.target === 'email') {
        ...
        if (image.width > 600) {
          const newImageDimensions = getResizedImageDimensions(image, {
            width: 600
          });
